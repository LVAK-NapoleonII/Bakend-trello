...
const removeMember = async (req, res) => {
  try {
    ...
    if (board.owner.toString() === userId) return res.status(400).json({ message: "Không thể xóa chủ phòng!" });

    const member = board.members.find((m) => m.user.toString() === userId && m.isActive);
    if (!member) return res.status(400).json({ message: "Người dùng không phải thành viên active!" });
